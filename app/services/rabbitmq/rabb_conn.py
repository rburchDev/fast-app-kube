...
class Worker:
    ...
    def __init__(self,
        ...
        self.url = URL(f"amqp://{self.rabbit_username}:{self.rabbit_password}@{self.rabbit_server}:{self.rabbit_port}/")
        self.data = {"Rabbit": self.url, "AIORMQ": [self.conn, self.channel, self.declare_ok, self.conn_rpc_consume,
    ...
    async def rpc_on_message(self, message: DeliveredMessage) -> None:
        ...
        await message.channel.basic_publish(
            body=str(n).encode(), routing_key=message.header.properties.reply_to,
    ...
    async def start_channel(self) -> tuple:
        ...
        if self.conn is None:
            ...
            self.declare_ok = await self.channel.queue_declare(queue="rabbit", exclusive=False, auto_delete=True)
    ...
    async def start_channel_rpc_consume(self) -> aiormq:
        if self.conn_rpc_consume is None:
            ...
            self.declare_ok_rpc_consume = await self.channel_rpc_consume.queue_declare(queue='rpc_queue')
    ...
    async def start_channel_rpc_publish(self) -> tuple:
        if self.conn_rpc_publish is None:
            ...
            self.declare_ok_rpc_publish = await self.channel_rpc_publish.queue_declare(queue="rpc_reply_queue",
                                                                                       exclusive=False,
                                                                                       auto_delete=True)
...
class Broker(Worker):
    def __init__(self):
        ...
        super().__init__(rabbit_server=os.environ['MY_POD_NAME'],
                         rabbit_port=5672,
    ...
    async def send_message(self, body: bytes, routing_key: str, exchange: str) -> None:
        try:
            ...
            await self.channel.basic_publish(body=body, routing_key=routing_key, exchange=exchange)
    ...
    async def rpc_reply_consume(self) -> None:
        try:
            ...
            await self.channel_rpc_publish.basic_consume(queue="rpc_reply_queue",
